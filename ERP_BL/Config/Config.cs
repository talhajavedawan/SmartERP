...
namespace ERP_BL.Config
{
    public class Config : IConfig
    {
        ...
        {
            List<string> list = new List<string>();
            string sqlPassword = Environment.GetEnvironmentVariable("SMARTERP_SQL_PASSWORD");

            if (!winAuth && string.IsNullOrWhiteSpace(sqlPassword))
            {
                ERP_BL.Databases.SystemLog.LogError(this.GetType(), "SQL authentication requested but SMARTERP_SQL_PASSWORD is not configured.");
                return list;
            }

            list = server.connectLANServer(serverName, winAuth, sqlPassword);
            if (list == null)
            {
