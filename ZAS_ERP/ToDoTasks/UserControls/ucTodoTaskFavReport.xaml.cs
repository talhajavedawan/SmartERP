...
{
    /// <summary>
    /// Interaction logic for ucTodoTaskFavReport.xaml
    /// </summary>
    public partial class ucTodoTaskFavReport : UserControl
    {
        ...
        public ucTodoTaskFavReport()
        {
            InitializeComponent();
        }
        public ucTodoTaskFavReport(GridReport reportToEdit, string title)
        {
            InitializeComponent();
            report = reportToEdit;
            reportTitle = title;
                ...
                if (rowHandle >= 0 && _step != null && _step.Id > 0)
                {
                    ...
                    if (_step.searchedFromDate != null && _step.searchedToDate != null /*&&  !String.IsNullOrEmpty(_step.SOField)*/)
                    {
                        if (_step.taskGroup.CompaniesBulk != null && _step.taskGroup.CompaniesBulk.Count() > 0 && _step.taskGroup.DepartmentsBulk != null && _step.taskGroup.DepartmentsBulk.Count() > 0)
                        {
                            ...
                            var systemMarginSE = supplyCCCSO.Sum(x => Convert.ToDecimal(x.SystemMargin) * Convert.ToDecimal(x.ccSER)) + nonPrincipalSO.Sum(x => Convert.ToDecimal(x.SystemMargin) * Convert.ToDecimal(x.marketExchangerate)) + principalSO.Sum(x => Convert.ToDecimal(x.SystemMargin) * Convert.ToDecimal(x.marketExchangerate));
